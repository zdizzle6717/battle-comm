...
class GameSystemForm extends React.Component {
	constructor() {
		this.state = {
			'faction': {},
			'gameSystem': {
	...
	componentDidMount() {
		...
		if (this.props.gameSystemId) {
			GameSystemService.get(this.props.gameSystemId).then((gameSystem) => {
				this.setState({
	...
	handleSaveGameSystem() {
		...
		let method = this.props.gameSystemId ? 'update' : 'create';
	...
	handleSaveFaction() {
		let data {
			...
			'GameSystemId': this.props.gameSystemId
		};
			...
			<Form name="gameSystemForm" submitText={this.state.newGameSystem ? 'Create Game System' : 'Update Game System'} handleSubmit={this.handleSaveGameSystem}>
				...
				<div className="row">
					<div className="form-group small-12 medium-4 columns">
						...
						<FileUpload name="gameSystemPhoto" value={this.state.gameSystem.File} handleFileUpload={this.handleFileUpload} maxFiles={1} />
					</div>
					<div className="form-group small-12 medium-4 columns">
...
GameSystemForm.propTypes = {
	'gameSystemId': React.PropTypes.number
}

GameSystemForm.defaultProps = {
}

export default connect(mapStateToProps, mapDispatchToProps)(GameSystemForm);
