...
export default class ManufacturerForm extends React.Component {
	...
	componentDidMount() {
		if (this.props.manufacturerId) {
			ManufacturerService.get(this.props.manufacturerId).then((manufacturer) => {
				this.setState({
	...
	handleSubmit() {
		let method = this.props.manufacturerId ? 'update': 'create';
			...
			<Form name="manufacturerForm" submitText={this.state.newGameSystem ? 'Create Manufacturer' : 'Update Manufacturer'} handleSubmit={this.handleSubmit}>
				<div className="row">
					...
					<div className="form-group small-12 medium-3 columns">
						...
						<FileUpload name="gameSystemPhoto" value={this.state.manufacturer.File} handleFileUpload={this.handleFileUpload} maxFiles={1} />
					</div>
				</div>
				<div className="row">
...
ManufacturerForm.propTypes = {
	'manufacturerId': React.PropTypes.number
}
