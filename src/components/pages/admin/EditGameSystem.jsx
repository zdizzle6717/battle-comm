...
class EditGameSystem extends React.Component {
	constructor() {
		super();
	...
	handleSaveGameSystem(e) {
		...
		GameSystemService[method]((method === 'update' ? system.id : system), (method === 'update' ? system : null)).then((gameSystem) => {
			...
			if (method === 'update') {
				...
				this.props.history.push(`/admin/game-systems/edit/${gameSystem.id}`);
	...
	handleSaveFaction() {
		let data = {
			...
			'GameSystemId': this.props.match.params.gameSystemId
		};
		...
		const alerts = {
			...
			'notFound': () => {
				this.props.addAlert({
					...
					'message': `No game system found with id, ${this.props.match.params.gameSystemId}`,
					'type': 'error',
					'delay': 3000
				});
			},
...
export default withRouter(connect(mapStateToProps, mapDispatchToProps)(EditGameSystem));
