            ...
            <form enctype="multipart/form-data"  class="Basic_Default" id="Update_Basic_Default" name="Update_Basic_Default" method="post" action="<?php echo(htmlentities($_SERVER["PHP_SELF"], ENT_QUOTES)."?".$_SERVER["QUERY_STRING"]); ?>">
              <fieldset class="Basic_Default" id="Update">
                ...
                <div class="lineGroup"> <label for="game_system_Title" class="sublabel" > Title:<span class="requiredIndicator">&nbsp;*</span></label>
                  <input id="game_system_Title" name="game_system_Title" type="text" value="<?php echo((isset($_GET["invalid"])?ValidatedField("gamesystemupdate","game_system_Title"):"".$row_WADAgame_system["game_system_Title"]."")); ?>" class="formTextfield_Large" tabindex="1" title="Please enter a value." required>
                  <?php
if (ValidatedField('gamesystemupdate','gamesystemupdate'))  {
  if ((strpos((",".ValidatedField("gamesystemupdate","gamesystemupdate").","), "," . "1" . ",") !== false || "1" == ""))  {
    if (!(false))  {
?>
                    <span class="serverInvalidState" id="game_system_Title_ServerError">Please enter a value.</span>
                    <?php //WAFV_Conditional game_system_update.php gamesystemupdate(1:)
    }
  }
}?>
                Please enter a value.</div> 
...
if (ValidatedField('gamesystemupdate','gamesystemupdate'))  {
  if ((strpos((",".ValidatedField("gamesystemupdate","gamesystemupdate").","), "," . "2" . ",") !== false || "2" == ""))  {
    if (!(false))  {
?>
                    <span class="serverInvalidState" id="game_system_official_url_ServerError">Please enter a value.</span>
                    <?php //WAFV_Conditional game_system_update.php gamesystemupdate(2:)
    }
  }
}?>
                Please enter a value.</div> 
                ...
                <div class="lineGroup"> <label for="game_logo_thumb" class="sublabel" > Logo thumbnail:</label>
                <img src="../uploads/gameSystems/thumbnails/<?php echo $row_WADAgame_system['game_logo']; ?>" alt="<?php echo $row_WADAgame_system['game_system_Title']; ?> thumbnail" />     </div> 
...
do {  
?>
                    <option value="<?php echo $row_WADAMenugames_category['game_category']?>"<?php if (!(strcmp($row_WADAMenugames_category['game_category'], (isset($_GET["invalid"])?ValidatedField("gamesystemupdate","games_category"):"".$row_WADAgame_system["games_category"]."")))) {echo "selected=\"selected\"";} ?>><?php echo $row_WADAMenugames_category['game_category']?></option>
                    <?php
...
?>
                ...
                <div class="lineGroup"> <label for="noOfPlayers" class="sublabel" > Number of Players:<span class="requiredIndicator">&nbsp;*</span></label>
                  <input id="noOfPlayers" name="noOfPlayers" type="text" value="<?php echo((isset($_GET["invalid"])?ValidatedField("gamesystemupdate","noOfPlayers"):"".$row_WADAgame_system["noOfPlayers"]."")); ?>" class="formTextfield_Small" tabindex="8" title="Please enter a value." required>
                  <?php
if (ValidatedField('gamesystemupdate','gamesystemupdate'))  {
  if ((strpos((",".ValidatedField("gamesystemupdate","gamesystemupdate").","), "," . "3" . ",") !== false || "3" == ""))  {
    if (!(false))  {
?>
                    <span class="serverInvalidState" id="noOfPlayers_ServerError">Please enter a value.</span>
                    <?php //WAFV_Conditional game_system_update.php gamesystemupdate(3:)
    }
  }
}?>
                </div> 
