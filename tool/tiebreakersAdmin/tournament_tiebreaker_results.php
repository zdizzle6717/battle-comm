...
//Recordset: WADAtournament_tiebreaker;
//Searchpage: tournament_tiebreaker_search.php;
//Form: WADASearchForm;
...
if (isset($_GET["Search"]) || isset($_GET["Search_x"])) {
  ...
  $WADbSearch1->addComparisonFromEdit("tiebreaker_name","tiebreaker_name","AND","Includes",0);
  $WADbSearch1->addComparisonFromEdit("match_id","match_id","AND","=",1);
  $WADbSearch1->addComparisonFromEdit("Game Title","Game Title","AND","Includes",0);

  //save the query in a session variable
  if (1 == 1) {
    $_SESSION["WADbSearch1_tournament_tiebreaker_results"]=$WADbSearch1->whereClause;
  }
}
else     {
  $WADbSearch1 = new FilterDef;
  ...
  if (1 == 1)     {
    if (isset($_SESSION["WADbSearch1_tournament_tiebreaker_results"]) && $_SESSION["WADbSearch1_tournament_tiebreaker_results"] != "")     {
      $WADbSearch1->whereClause = $_SESSION["WADbSearch1_tournament_tiebreaker_results"];
    }
    else     {
...
?>
<?php
$maxRows_WADAtournament_tiebreaker = 10;
...
$query_WADAtournament_tiebreaker = "SELECT tourney_tiebreaker_id, tiebreaker_name, match_id, `Game Title`, tiebreaker_conditions, point_value FROM tournament_tiebreaker";
setQueryBuilderSource($query_WADAtournament_tiebreaker,$WADbSearch1,false);
...
$WADAtournament_tiebreaker = mysql_query($query_limit_WADAtournament_tiebreaker, $local_local) or die(mysql_error());
...
if (isset($_GET['totalRows_WADAtournament_tiebreaker'])) {
  ...
  $all_WADAtournament_tiebreaker = mysql_query($query_WADAtournament_tiebreaker, $local_local);
  ...
  <div class="WADAResults">
    <div class="WADAResultsNavigation"><wa:insertbutton>
			<div class="WADAResultsInsertButton">
				<input type="button" value="Insert" class="" id="Insert" name="Insert" onclick="document.location.href='tournament_tiebreaker_insert.php';" />
			</div></wa:insertbutton>
    ...
    <table class="WADAResultsTable" border="0" cellpadding="0" cellspacing="0" width="100%">
      <tr>
        <th class="WADAResultsTableHeader">Tiebreaker/Mission Name:</th>
        <th class="WADAResultsTableHeader">Round ID:</th>
        <th class="WADAResultsTableHeader">Game Title:</th>
        <th class="WADAResultsTableHeader">Tiebreaker/Mission Conditions:</th>
...
<?php echo(isset($_GET["pageNum_WADAtournament_tiebreaker"])?"&pageNum_WADAtournament_tiebreaker=".intval($_GET["pageNum_WADAtournament_tiebreaker"]):""); ?>" ><?php echo($row_WADAtournament_tiebreaker['tiebreaker_name']); ?></a></td>
          <td class="WADAResultsTableCell"><?php echo($row_WADAtournament_tiebreaker['match_id']); ?></td>
          <td class="WADAResultsTableCell"><?php echo($row_WADAtournament_tiebreaker['Game Title']); ?></td>
          <td class="WADAResultsTableCell"><?php echo($row_WADAtournament_tiebreaker['tiebreaker_conditions']); ?></td>
          <td class="WADAResultsTableCell"><?php echo($row_WADAtournament_tiebreaker['point_value']); ?></td>
          
		<td class="WADAResultsEditButtons" nowrap="nowrap"><input type="button" class="formButton ResultsPageButton Dark DetailButton" value="" onclick="document.location.href='tournament_tiebreaker_detail.php?tourney_tiebreaker_id=<?php echo($row_WADAtournament_tiebreaker['tourney_tiebreaker_id']); ?>
<?php echo(isset($_GET["pageNum_WADAtournament_tiebreaker"])?"&pageNum_WADAtournament_tiebreaker=".intval($_GET["pageNum_WADAtournament_tiebreaker"]):""); ?>';"/><input type="button" class="formButton ResultsPageButton Dark UpdateButton" value="" onclick="document.location.href='tournament_tiebreaker_update.php?tourney_tiebreaker_id=<?php echo($row_WADAtournament_tiebreaker['tourney_tiebreaker_id']); ?>
<?php echo(isset($_GET["pageNum_WADAtournament_tiebreaker"])?"&pageNum_WADAtournament_tiebreaker=".intval($_GET["pageNum_WADAtournament_tiebreaker"]):""); ?>';"/><input type="button" class="formButton ResultsPageButton Dark DeleteButton" value="" onclick="document.getElementById('WADADeleteRecordID').value=<?php echo($row_WADAtournament_tiebreaker['tourney_tiebreaker_id']); ?>;document.getElementById('deleteBox').style.display = 'block';document.getElementById('deleteMessage').style.display = 'table';" /></td>
        </tr>
<?php } while ($row_WADAtournament_tiebreaker = mysql_fetch_assoc($WADAtournament_tiebreaker)); ?>
    ...
    <div class="WADAResultsNavigation">
      ...
      <div class="WADAResultsNavBottom">
        <table border="0" cellpadding="0" cellspacing="0" class="WADAResultsNavTable">
          <tr valign="middle">
            <td class="WADAResultsNavButtonCell" nowrap="nowrap"><?php if ($pageNum_WADAtournament_tiebreaker > 0) { // Show if not first page ?><input type="button" value="<<" class="formButton ResultsNavButton Dark" id="First_2" name="First_2" onclick="document.location.href='<?php printf("%s?pageNum_WADAtournament_tiebreaker=%d%s", $currentPage, 0, $queryString_WADAtournament_tiebreaker); ?>';" /><?php } // Show if not first page ?></td>
            <td class="WADAResultsNavButtonCell" nowrap="nowrap"><?php if ($pageNum_WADAtournament_tiebreaker > 0) { // Show if not first page ?><input type="button" value="<" class="formButton ResultsNavButton Dark" id="Previous_2" name="Previous_2" onclick="document.location.href='<?php printf("%s?pageNum_WADAtournament_tiebreaker=%d%s", $currentPage, max(0, $pageNum_WADAtournament_tiebreaker - 1), $queryString_WADAtournament_tiebreaker); ?>';" /><?php } // Show if not first page ?></td>
            <td class="WADAResultsNavButtonCell" nowrap="nowrap"><?php if ($pageNum_WADAtournament_tiebreaker < $totalPages_WADAtournament_tiebreaker) { // Show if not last page ?><input type="button" value=">" class="formButton ResultsNavButton Dark" id="Next_2" name="Next_2" onclick="document.location.href='<?php printf("%s?pageNum_WADAtournament_tiebreaker=%d%s", $currentPage, min($totalPages_WADAtournament_tiebreaker, $pageNum_WADAtournament_tiebreaker + 1), $queryString_WADAtournament_tiebreaker); ?>';" /><?php } // Show if not last page ?></td>
            <td class="WADAResultsNavButtonCell" nowrap="nowrap"><?php if ($pageNum_WADAtournament_tiebreaker < $totalPages_WADAtournament_tiebreaker) { // Show if not last page ?><input type="button" value=">>" class="formButton ResultsNavButton Dark" id="Last_2" name="Last_2" onclick="document.location.href='<?php printf("%s?pageNum_WADAtournament_tiebreaker=%d%s", $currentPage, $totalPages_WADAtournament_tiebreaker, $queryString_WADAtournament_tiebreaker); ?>';" /><?php } // Show if not last page ?></td>
          </tr>
        </table>
      </div>
    </div>
