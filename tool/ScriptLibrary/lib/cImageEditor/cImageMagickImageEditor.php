...
class cImageMagickImageEditor extends cImageEditor
{
	function cImageMagickImageEditor( $sFileName = "" )
	{
	...
	function crop( $nStartX, $nStartY, $nEndX, $nEndY )
	{
		...
		$bSuccess = imagick_crop( $$this->rImage, 0, 0, $nStartX, $nStartY, $nWidth, $nHeight );
	...
	function text(  $sPosition, $sText, $nFontSize = 24, $sFontStyle = 'normal', $sFontColor = '#ffffff' )
	{
		...
		if ( !imagick_setfontface($this->rImage, $this->sUsedFont) ) {
			return false;
		}
		...
		//we BADLY DEPEND ON GD to get the size of the text box
		//otherwise we die
...
//			return false;
//		}
//      GP: Crash bug confirmed ...
//
//		if (!@imagick_setfillcolor($this->rImage, $sFontColor)) {
//				$reason      = imagick_failedreason( $this->rImage ) ;
//                $description = imagick_faileddescription( $this->rImage ) ;
//				echo 'error: '.$reason.'-'.$description.'<br>';
//		} 

		$bSuccess = imagick_drawannotation($this->rImage, $nTextLeft, $nTextTop, $sText);
		return $bSuccess;
	}	
	 ...
	 * @return bool
	 */
	function textabs(  $nPosX, $nPosY, $sText, $nFontSize = 24, $sFontStyle = 'normal', $sFontColor = 'ffffff')
	{
		# Prepare the image, if not prepared
		if ( is_null( $this->rImage ) )
		{
...
//		}
	...
	function saveJPEG( $sFileName = '', $nQuality = 75, $bgColor = '' )
	{
		...
		if ($bgColor != '')
		{
			$im = imagick_getcanvas($bgColor, $tWidth, $tHeight);
			imagick_composite($im, IMAGICK_COMPOSITE_OP_NONE, $this->rImage, 0, 0);
			$bSuccess = (bool)imagick_writeimage($im, $sFileName);
			imagick_destroyhandle($im);
		}
		else
		{
	...
	function saveGIF( $sFileName = '', $bDitherFlag = false, $nColorsWanted = 0 )
	{
		...
		$sExtension = substr($sFileName, -3);
		if (strtolower($sExtension) == 'gif') {
			// We have the correct extension - go, go, go!
		}
		else {
			$sFileName .= '.gif';  // Set the right extension
		}
