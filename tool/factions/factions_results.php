...
?>
<?php
$maxRows_WADAfactions = 15;
$pageNum_WADAfactions = 0;
if (isset($_GET['pageNum_WADAfactions'])) {
  $pageNum_WADAfactions = $_GET['pageNum_WADAfactions'];
}
$startRow_WADAfactions = $pageNum_WADAfactions * $maxRows_WADAfactions;

mysql_select_db($database_local_local, $local_local);
$query_WADAfactions = "SELECT faction_id, faction_name, `game_system`.`game_system_Title` AS game_system_game_system_Title FROM factions LEFT JOIN game_system ON game_system.game_system_id = factions.game_system_id";
$query_limit_WADAfactions = sprintf("%s LIMIT %d, %d", $query_WADAfactions, $startRow_WADAfactions, $maxRows_WADAfactions);
$WADAfactions = mysql_query($query_limit_WADAfactions, $local_local) or die(mysql_error());
$row_WADAfactions = mysql_fetch_assoc($WADAfactions);

if (isset($_GET['totalRows_WADAfactions'])) {
  $totalRows_WADAfactions = $_GET['totalRows_WADAfactions'];
} else {
  $all_WADAfactions = mysql_query($query_WADAfactions, $local_local);
  $totalRows_WADAfactions = mysql_num_rows($all_WADAfactions);
}
$totalPages_WADAfactions = ceil($totalRows_WADAfactions/$maxRows_WADAfactions)-1;
?>
<?php
$queryString_WADAfactions = "";
if (!empty($_SERVER['QUERY_STRING'])) {
  $params = explode("&", $_SERVER['QUERY_STRING']);
  $newParams = array();
  foreach ($params as $param) {
    if (stristr($param, "pageNum_WADAfactions") == false && 
        stristr($param, "totalRows_WADAfactions") == false) {
      array_push($newParams, $param);
    }
  }
  if (count($newParams) != 0) {
    $queryString_WADAfactions = "&" . htmlentities(implode("&", $newParams));
  }
}
$queryString_WADAfactions = sprintf("&totalRows_WADAfactions=%d%s", $totalRows_WADAfactions, $queryString_WADAfactions);
?>
<?php 
  ...
  <div class="WADAResults">
    <div class="WADAResultsNavigation"><wa:insertbutton>
			<div class="WADAResultsInsertButton">
				<input type="button" value="Insert" class="" id="Insert" name="Insert" onclick="document.location.href='factions_insert.php';" />
			</div></wa:insertbutton>
      <div class="WADAResultsCount">Records  <?php echo ($startRow_WADAfactions + 1) ?> to  <?php echo min($startRow_WADAfactions + $maxRows_WADAfactions, $totalRows_WADAfactions) ?> of  <?php echo $totalRows_WADAfactions ?></div>
			<div class="WADAResultsNavTop">
				<table border="0" cellpadding="0" cellspacing="0" class="WADAResultsNavTable">
					<tr valign="middle">
            <td class="WADAResultsNavButtonCell" nowrap="nowrap"><?php if ($pageNum_WADAfactions > 0) { // Show if not first page ?><input type="button" value="<<" class="formButton ResultsNavButton Dark" id="First" name="First" onclick="document.location.href='<?php printf("%s?pageNum_WADAfactions=%d%s", $currentPage, 0, $queryString_WADAfactions); ?>';" /><?php } // Show if not first page ?></td>
            <td class="WADAResultsNavButtonCell" nowrap="nowrap"><?php if ($pageNum_WADAfactions > 0) { // Show if not first page ?><input type="button" value="<" class="formButton ResultsNavButton Dark" id="Previous" name="Previous" onclick="document.location.href='<?php printf("%s?pageNum_WADAfactions=%d%s", $currentPage, max(0, $pageNum_WADAfactions - 1), $queryString_WADAfactions); ?>';" /><?php } // Show if not first page ?></td>
            <td class="WADAResultsNavButtonCell" nowrap="nowrap"><?php if ($pageNum_WADAfactions < $totalPages_WADAfactions) { // Show if not last page ?><input type="button" value=">" class="formButton ResultsNavButton Dark" id="Next" name="Next" onclick="document.location.href='<?php printf("%s?pageNum_WADAfactions=%d%s", $currentPage, min($totalPages_WADAfactions, $pageNum_WADAfactions + 1), $queryString_WADAfactions); ?>';" /><?php } // Show if not last page ?></td>
            <td class="WADAResultsNavButtonCell" nowrap="nowrap"><?php if ($pageNum_WADAfactions < $totalPages_WADAfactions) { // Show if not last page ?><input type="button" value=">>" class="formButton ResultsNavButton Dark" id="Last" name="Last" onclick="document.location.href='<?php printf("%s?pageNum_WADAfactions=%d%s", $currentPage, $totalPages_WADAfactions, $queryString_WADAfactions); ?>';" /><?php } // Show if not last page ?></td>
					</tr>
				</table>
			</div>
    </div>
    ...
    <table class="WADAResultsTable" border="0" cellpadding="0" cellspacing="0" width="100%">
      <tr>
        <th class="WADAResultsTableHeader">Faction Name:</th>
        <th class="WADAResultsTableHeader">Game Systsem:</th>
...
<?php do { ?>
        <tr class="<?php echo($WARRT_AltClass1->getClass(true)); ?>">
          <td class="WADAResultsTableCell"><a href="factions_detail.php?faction_id=<?php echo($row_WADAfactions['faction_id']); ?>
<?php echo(isset($_GET["pageNum_WADAfactions"])?"&pageNum_WADAfactions=".intval($_GET["pageNum_WADAfactions"]):""); ?>" ><?php echo($row_WADAfactions['faction_name']); ?></a></td>
          <td class="WADAResultsTableCell"><?php echo($row_WADAfactions['game_system_game_system_Title']); ?></td>
          
		<td class="WADAResultsEditButtons" nowrap="nowrap"><input type="button" class="formButton ResultsPageButton Dark DetailButton" value="" onclick="document.location.href='factions_detail.php?faction_id=<?php echo($row_WADAfactions['faction_id']); ?>
<?php echo(isset($_GET["pageNum_WADAfactions"])?"&pageNum_WADAfactions=".intval($_GET["pageNum_WADAfactions"]):""); ?>';"/><input type="button" class="formButton ResultsPageButton Dark UpdateButton" value="" onclick="document.location.href='factions_update.php?faction_id=<?php echo($row_WADAfactions['faction_id']); ?>
<?php echo(isset($_GET["pageNum_WADAfactions"])?"&pageNum_WADAfactions=".intval($_GET["pageNum_WADAfactions"]):""); ?>';"/><input type="button" class="formButton ResultsPageButton Dark DeleteButton" value="" onclick="document.getElementById('WADADeleteRecordID').value=<?php echo($row_WADAfactions['faction_id']); ?>;document.getElementById('deleteBox').style.display = 'block';document.getElementById('deleteMessage').style.display = 'table';" /></td>
        </tr>
<?php } while ($row_WADAfactions = mysql_fetch_assoc($WADAfactions)); ?>
    </table>
	</div>
    <div class="WADAResultsNavigation">
      <div class="WADAResultsCountBottom">Records  <?php echo ($startRow_WADAfactions + 1) ?> to  <?php echo min($startRow_WADAfactions + $maxRows_WADAfactions, $totalRows_WADAfactions) ?> of  <?php echo $totalRows_WADAfactions ?></div>
      <div class="WADAResultsNavBottom">
        <table border="0" cellpadding="0" cellspacing="0" class="WADAResultsNavTable">
          <tr valign="middle">
            <td class="WADAResultsNavButtonCell" nowrap="nowrap"><?php if ($pageNum_WADAfactions > 0) { // Show if not first page ?><input type="button" value="<<" class="formButton ResultsNavButton Dark" id="First_2" name="First_2" onclick="document.location.href='<?php printf("%s?pageNum_WADAfactions=%d%s", $currentPage, 0, $queryString_WADAfactions); ?>';" /><?php } // Show if not first page ?></td>
            <td class="WADAResultsNavButtonCell" nowrap="nowrap"><?php if ($pageNum_WADAfactions > 0) { // Show if not first page ?><input type="button" value="<" class="formButton ResultsNavButton Dark" id="Previous_2" name="Previous_2" onclick="document.location.href='<?php printf("%s?pageNum_WADAfactions=%d%s", $currentPage, max(0, $pageNum_WADAfactions - 1), $queryString_WADAfactions); ?>';" /><?php } // Show if not first page ?></td>
            <td class="WADAResultsNavButtonCell" nowrap="nowrap"><?php if ($pageNum_WADAfactions < $totalPages_WADAfactions) { // Show if not last page ?><input type="button" value=">" class="formButton ResultsNavButton Dark" id="Next_2" name="Next_2" onclick="document.location.href='<?php printf("%s?pageNum_WADAfactions=%d%s", $currentPage, min($totalPages_WADAfactions, $pageNum_WADAfactions + 1), $queryString_WADAfactions); ?>';" /><?php } // Show if not last page ?></td>
            <td class="WADAResultsNavButtonCell" nowrap="nowrap"><?php if ($pageNum_WADAfactions < $totalPages_WADAfactions) { // Show if not last page ?><input type="button" value=">>" class="formButton ResultsNavButton Dark" id="Last_2" name="Last_2" onclick="document.location.href='<?php printf("%s?pageNum_WADAfactions=%d%s", $currentPage, $totalPages_WADAfactions, $queryString_WADAfactions); ?>';" /><?php } // Show if not last page ?></td>
          </tr>
        </table>
      </div>
    </div>
