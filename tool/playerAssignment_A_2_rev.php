...
$query_ActivePlayer = "SELECT * FROM players WHERE active = 'yes' ORDER BY totalPoints DESC";
...
$row_ActivePlayer = mysql_fetch_assoc($ActivePlayer);
$totalRows_ActivePlayer = mysql_num_rows($ActivePlayer);
$query_ActivePlayer = "SELECT * FROM players WHERE active = 'yes' ORDER BY players.totalPoints DESC";
$ActivePlayer = mysql_query($query_ActivePlayer, $local_local) or die(mysql_error());
$row_ActivePlayer = mysql_fetch_assoc($ActivePlayer);
$totalRows_ActivePlayer = mysql_num_rows($ActivePlayer);
$query_ActivePlayer = "SELECT * FROM players WHERE active = 'yes' ORDER BY totalPoints DESC";
...
$colname_tourneyPlayers = "10";
if (isset($_GET['tourney'])) {
  $colname_tourneyPlayers = $_GET['tourney'];
}
mysql_select_db($database_local_local, $local_local);
$query_tourneyPlayers = sprintf("SELECT * FROM tournament_game_player WHERE tournament_id = %s AND tournament_game_player.results_approved = 'yes'", GetSQLValueString($colname_tourneyPlayers, "int"));
...
<meta charset="utf-8">
<title>Assign Players (based on Total Points):<?php echo $row_tournamentGameJoin['tournament_name']; ?></title>
...
<table width="90%" border="1">
  <tr>
    ...
    <th scope="col">Previous Results</th>
    </tr>
   <?php
...
?><tr>
  ...
  <input name="playerID_<?php echo $RepeatSelectionCounter_1; ?>" type="hidden" id="playerID_<?php echo $RepeatSelectionCounter_1; ?>" value="<?php echo $row_ActivePlayer['playerId']; ?>"></td>
    ...
    <td><input name="Session_<?php echo $RepeatSelectionCounter_1; ?>" type="text" id="Game_<?php echo $RepeatSelectionCounter_1; ?>" value="Game  "><input name="tournamentID_<?php echo $RepeatSelectionCounter_1; ?>" type="hidden" id="tournamentID_<?php echo $RepeatSelectionCounter_1; ?>" value="<?php echo $row_tournamentGameJoin['tournament_id']; ?>"></td>
    ...
    <td><?php echo $row_ActivePlayer['totalPoints']; ?></td>
    </tr><?php
...
<p>
  <?php
	...
	while($RepeatSelectionCounter_3_IterationsRemaining--){
		if($RepeatSelectionCounterBasedLooping_3 || $row_ActivePlayer){
?>
<table width="90%" border="1">
  <tr>
    <th scope="col">Handle</th>
  ...
  <tr>
    <td><?php echo $row_ActivePlayer['playerHandle']; ?></td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td><?php echo $row_ActivePlayer['totalPoints']; ?></td>
...
<?php } // RepeatSelectionCounter_3 End Alternate Content
		if(!$RepeatSelectionCounterBasedLooping_3 && $RepeatSelectionCounter_3_IterationsRemaining != 0){
			if(!$row_ActivePlayer && $RepeatSelectionCounter_3_Iterations == -1){$RepeatSelectionCounter_3_IterationsRemaining = 0;}
			$row_ActivePlayer = mysql_fetch_assoc($ActivePlayer);
		}
