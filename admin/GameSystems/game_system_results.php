...
$query_WADAgame_system = "SELECT game_system_id, game_system_Title, games_category FROM game_system ORDER BY game_system_Title ASC";
...
$WADAgame_system = mysql_query($query_limit_WADAgame_system, $local) or die(mysql_error());
...
if (isset($_GET['totalRows_WADAgame_system'])) {
  ...
  $all_WADAgame_system = mysql_query($query_WADAgame_system);
  ...
  <div class="WADAResults">
    <div class="WADAResultsNavigation"><wa:insertbutton>
			<div class="WADAResultsInsertButton">
				<input type="button" value="Insert" class="formButton Spacious" id="Insert" name="Insert" onclick="document.location.href='game_system_insert.php';" />
			</div></wa:insertbutton>
      <div class="WADAResultsCount">Records  <?php echo ($startRow_WADAgame_system + 1) ?> to  <?php echo min($startRow_WADAgame_system + $maxRows_WADAgame_system, $totalRows_WADAgame_system) ?> of  <?php echo $totalRows_WADAgame_system ?></div>
			<div class="WADAResultsNavTop">
				<table border="0" cellpadding="0" cellspacing="0" class="WADAResultsNavTable">
					<tr valign="middle">
            <td class="WADAResultsNavButtonCell" nowrap="nowrap"><?php if ($pageNum_WADAgame_system > 0) { // Show if not first page ?><input type="button" value="<<" class="formButton ResultsNavButton Spacious" id="First" name="First" onclick="document.location.href='<?php printf("%s?pageNum_WADAgame_system=%d%s", $currentPage, 0, $queryString_WADAgame_system); ?>';" /><?php } // Show if not first page ?></td>
            <td class="WADAResultsNavButtonCell" nowrap="nowrap"><?php if ($pageNum_WADAgame_system > 0) { // Show if not first page ?><input type="button" value="<" class="formButton ResultsNavButton Spacious" id="Previous" name="Previous" onclick="document.location.href='<?php printf("%s?pageNum_WADAgame_system=%d%s", $currentPage, max(0, $pageNum_WADAgame_system - 1), $queryString_WADAgame_system); ?>';" /><?php } // Show if not first page ?></td>
            <td class="WADAResultsNavButtonCell" nowrap="nowrap"><?php if ($pageNum_WADAgame_system < $totalPages_WADAgame_system) { // Show if not last page ?><input type="button" value=">" class="formButton ResultsNavButton Spacious" id="Next" name="Next" onclick="document.location.href='<?php printf("%s?pageNum_WADAgame_system=%d%s", $currentPage, min($totalPages_WADAgame_system, $pageNum_WADAgame_system + 1), $queryString_WADAgame_system); ?>';" /><?php } // Show if not last page ?></td>
            <td class="WADAResultsNavButtonCell" nowrap="nowrap"><?php if ($pageNum_WADAgame_system < $totalPages_WADAgame_system) { // Show if not last page ?><input type="button" value=">>" class="formButton ResultsNavButton Spacious" id="Last" name="Last" onclick="document.location.href='<?php printf("%s?pageNum_WADAgame_system=%d%s", $currentPage, $totalPages_WADAgame_system, $queryString_WADAgame_system); ?>';" /><?php } // Show if not last page ?></td>
					</tr>
				</table>
			</div>
    </div>
...
<?php echo(isset($_GET["pageNum_WADAgame_system"])?"&pageNum_WADAgame_system=".intval($_GET["pageNum_WADAgame_system"]):""); ?>" ><?php echo($row_WADAgame_system['game_system_Title']); ?></a></td>
		...
		<td class="WADAResultsEditButtons" nowrap="nowrap"><input type="button" class="formButton ResultsPageButton Spacious DetailButton" value="" onclick="document.location.href='game_system_detail.php?game_system_id=<?php echo($row_WADAgame_system['game_system_id']); ?>
<?php echo(isset($_GET["pageNum_WADAgame_system"])?"&pageNum_WADAgame_system=".intval($_GET["pageNum_WADAgame_system"]):""); ?>';"/><input type="button" class="formButton ResultsPageButton Spacious UpdateButton" value="" onclick="document.location.href='game_system_update.php?game_system_id=<?php echo($row_WADAgame_system['game_system_id']); ?>
<?php echo(isset($_GET["pageNum_WADAgame_system"])?"&pageNum_WADAgame_system=".intval($_GET["pageNum_WADAgame_system"]):""); ?>';"/><input type="button" class="formButton ResultsPageButton Spacious DeleteButton" value="" onclick="document.getElementById('WADADeleteRecordID').value=<?php echo($row_WADAgame_system['game_system_id']); ?>;document.getElementById('deleteBox').style.display = 'block';document.getElementById('deleteMessage').style.display = 'table';" /></td>
        </tr>
<?php } while ($row_WADAgame_system = mysql_fetch_assoc($WADAgame_system)); ?>
    ...
    <div class="WADAResultsNavigation">
      ...
      <div class="WADAResultsNavBottom">
        <table border="0" cellpadding="0" cellspacing="0" class="WADAResultsNavTable">
          <tr valign="middle">
            <td class="WADAResultsNavButtonCell" nowrap="nowrap"><?php if ($pageNum_WADAgame_system > 0) { // Show if not first page ?><input type="button" value="<<" class="formButton ResultsNavButton Spacious" id="First_2" name="First_2" onclick="document.location.href='<?php printf("%s?pageNum_WADAgame_system=%d%s", $currentPage, 0, $queryString_WADAgame_system); ?>';" /><?php } // Show if not first page ?></td>
            <td class="WADAResultsNavButtonCell" nowrap="nowrap"><?php if ($pageNum_WADAgame_system > 0) { // Show if not first page ?><input type="button" value="<" class="formButton ResultsNavButton Spacious" id="Previous_2" name="Previous_2" onclick="document.location.href='<?php printf("%s?pageNum_WADAgame_system=%d%s", $currentPage, max(0, $pageNum_WADAgame_system - 1), $queryString_WADAgame_system); ?>';" /><?php } // Show if not first page ?></td>
            <td class="WADAResultsNavButtonCell" nowrap="nowrap"><?php if ($pageNum_WADAgame_system < $totalPages_WADAgame_system) { // Show if not last page ?><input type="button" value=">" class="formButton ResultsNavButton Spacious" id="Next_2" name="Next_2" onclick="document.location.href='<?php printf("%s?pageNum_WADAgame_system=%d%s", $currentPage, min($totalPages_WADAgame_system, $pageNum_WADAgame_system + 1), $queryString_WADAgame_system); ?>';" /><?php } // Show if not last page ?></td>
            <td class="WADAResultsNavButtonCell" nowrap="nowrap"><?php if ($pageNum_WADAgame_system < $totalPages_WADAgame_system) { // Show if not last page ?><input type="button" value=">>" class="formButton ResultsNavButton Spacious" id="Last_2" name="Last_2" onclick="document.location.href='<?php printf("%s?pageNum_WADAgame_system=%d%s", $currentPage, $totalPages_WADAgame_system, $queryString_WADAgame_system); ?>';" /><?php } // Show if not last page ?></td>
          </tr>
        </table>
      </div>
    </div>
