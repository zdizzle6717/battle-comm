...
$query_WADAuser_profile = "SELECT iduser_profile, userID, user_city, user_zip, accountActive FROM user_profile";
...
$WADAuser_profile = mysql_query($query_limit_WADAuser_profile, $local) or die(mysql_error());
...
if (isset($_GET['totalRows_WADAuser_profile'])) {
  ...
  $all_WADAuser_profile = mysql_query($query_WADAuser_profile, $local);
  ...
  <div class="WADAResults">
    <div class="WADAResultsNavigation"><wa:insertbutton>
			<div class="WADAResultsInsertButton">
				<input type="button" value="Insert" class="formButton Modular" id="Insert" name="Insert" onclick="document.location.href='user_profile_insert.php';" />
			</div></wa:insertbutton>
    ...
    <table class="WADAResultsTable" border="0" cellpadding="0" cellspacing="0" width="100%">
      <tr>
        <th class="WADAResultsTableHeader">userID:</th>
        <th class="WADAResultsTableHeader">City:</th>
        <th class="WADAResultsTableHeader">Zip Code:</th>
        <th class="WADAResultsTableHeader">Account Active:</th>
...
<?php do { ?>
        <tr class="<?php echo($WARRT_AltClass1->getClass(true)); ?>">
          <td class="WADAResultsTableCell"><?php echo($row_WADAuser_profile['userID']); ?></td>
          <td class="WADAResultsTableCell"><?php echo($row_WADAuser_profile['user_city']); ?></td>
          <td class="WADAResultsTableCell"><?php echo($row_WADAuser_profile['user_zip']); ?></td>
          <td class="WADAResultsTableCell"><?php echo($row_WADAuser_profile['accountActive']); ?></td>
          
		<td class="WADAResultsEditButtons" nowrap="nowrap"><input type="button" class="formButton ResultsPageButton Modular UpdateButton" value="" onclick="document.location.href='user_profile_update.php?iduser_profile=<?php echo($row_WADAuser_profile['iduser_profile']); ?>
<?php echo(isset($_GET["pageNum_WADAuser_profile"])?"&pageNum_WADAuser_profile=".intval($_GET["pageNum_WADAuser_profile"]):""); ?>';"/><input type="button" class="formButton ResultsPageButton Modular DeleteButton" value="" onclick="document.getElementById('WADADeleteRecordID').value=<?php echo($row_WADAuser_profile['iduser_profile']); ?>;document.getElementById('deleteBox').style.display = 'block';document.getElementById('deleteMessage').style.display = 'table';" /></td>
        </tr>
      ...
      <div class="WADAResultsNavBottom">
        <table border="0" cellpadding="0" cellspacing="0" class="WADAResultsNavTable">
          <tr valign="middle">
            ...
            <td class="WADAResultsNavButtonCell" nowrap="nowrap"><?php if ($pageNum_WADAuser_profile < $totalPages_WADAuser_profile) { // Show if not last page ?><input type="button" value=">" class="formButton ResultsNavButton Modular" id="Next_2" name="Next_2" onclick="document.location.href='<?php printf("%s?pageNum_WADAuser_profile=%d%s", $currentPage, min($totalPages_WADAuser_profile, $pageNum_WADAuser_profile + 1), $queryString_WADAuser_profile); ?>';" /><?php } // Show if not last page ?></td>
            <td class="WADAResultsNavButtonCell" nowrap="nowrap"><?php if ($pageNum_WADAuser_profile < $totalPages_WADAuser_profile) { // Show if not last page ?><input type="button" value=">>" class="formButton ResultsNavButton Modular" id="Last_2" name="Last_2" onclick="document.location.href='<?php printf("%s?pageNum_WADAuser_profile=%d%s", $currentPage, $totalPages_WADAuser_profile, $queryString_WADAuser_profile); ?>';" /><?php } // Show if not last page ?></td>
          </tr>
        </table>
      </div>
    </div>
