...
class fs extends Module
{
    ...
    public function removedir($options) {
        ...
        $this->emptydir((object)array('path' => $path));
    ...
    public function emptydir($options) {
        ...
        foreach ($files as $file) {
            ...
            if (FileSystem::isdir($entry)) {
                $this->removedir((object)array('path' => $entry));
    ...
    public function dir($options) {
		...
		foreach ($entries as $entry) {
            if (!$options->includeFolders && FileSystem::isdir($path . DIRECTORY_SEPARATOR . $entry)) {
                // skip folders
                continue;
            }
            ...
            if (count($allowedExtensions) === 0 || in_array(substr($entry, strrpos($entry, '.')), $allowedExtensions)) {
