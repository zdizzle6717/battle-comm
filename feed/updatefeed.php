...
/* dmxDataSet name "LoggedInUser" */
  ...
  /* dmxDataSet name "TableUpdateFeed" */
       jQuery.dmxDataSet(
         {"id": "TableUpdateFeed", "url": "../dmxDatabaseSources/TableUpdateFeedData.php", "data": {"limit": "15"}, "dataSourceType": "database", "dataType": "jsonp"}
       );
  /* END dmxDataSet name "TableUpdateFeed" */
...
<nav class="topnav top-menu" role="navigation">
	...
	<div class="element-input"><label class="title">Title</label><input class="large" name="title" type="text" id="title"></div>
	<div class="element-textarea"><label class="title">Description</label><textarea class="large" name="description" cols="20" rows="5" id="description"></textarea></div>
<div><input name="UpdateFeed" type="button" id="UpdateFeed" onClick="dmxDatabaseActionControl('run','TableUpdateFeed',{},this)" value="Update Feed"></div></p>
<div>

</div>
<div data-binding-id="repeat1" data-binding-repeat="{{TableUpdateFeed.data}}"><br>
{{title}}- {{description}}
</div>
                      </div>
                </div>
...
<div class="copyright">© 2015 Battle-Comm.com. All Rights Reserved.
  ...
  <h4>All copyrights belong to their respective owners. Images and text owned by other copyright holders are used here under the guidelines of the Fair Use provisions of United States Copyright Law.</h4>
    ...
    <script type="text/javascript">
  /* dmxDatabaseAction name "TableUpdateFeed" */
       jQuery.dmxDatabaseAction(
         {"id": "TableUpdateFeed", "url": "../dmxDatabaseActions/TableUpdateInsert.php", "data": {"title": "{{$FORM.title}}", "description": "{{$FORM.description}}"}, "beforeSend": "dmxDataBindingsAction('refresh','TableUpdateFeed',{});"}
       );
  /* END dmxDatabaseAction name "TableUpdateFeed" */
        </script>
    </body>
</html>